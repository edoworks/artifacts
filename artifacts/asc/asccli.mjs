#!/usr/bin/env node
// asccli.mjs — minimal CLI wrapper around ascclient.mjs.
// Usage:
//   ASC_KEY_PATH=/path/AuthKey_XXXXXXXXXX.p8 ASC_ISSUER_ID=<uuid> \
//     node asccli.mjs request --method GET --path /v1/apps
import { signJwt, ascRequest, resolveAscCredentials } from "./ascclient.mjs";
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    switch (flag) {
      ...
      case "-h": case "--help":
        process.stdout.write("Usage: node asccli.mjs request --method <M> --path <P> [--body <json>]\n");
        process.exit(0);
      default:
